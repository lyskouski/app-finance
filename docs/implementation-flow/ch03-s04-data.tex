...
truly relevant for each item within our application \issue{17}{}. Here's the trick! In our approach, we store the user 
input exactly as it is, especially in the context of financial applications, such as transactions. All structures 
(as bills, accounts, budgets, currencies rate, invoices, transfers, etc.) are derived from this core item and will be 
used at run-time only as a representation layer of those "transactions".
