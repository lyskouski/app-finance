...
\subsection{Defining Quality Gates}
\markboth{Implementing Core Functionality}{Defining Quality Gates}
...
\cite{Sanket19}, and an architectural mistake might cost `N x 30` on post-release phase, where N -- is an amount of
totally spent money for that feature.
...
In Flutter, `test`-function from the `flutter\_test`- package is utilized for writing unit tests. By writing them, 
all the defined function calls within files following the *\_test.dart pattern are automatically recognized by the 
`flutter test`-command line execution. Alternatively, all tests are shown in "Test Explorer" of IDE and can be
...
\noindent By typing `flutter test` in the command line we would see something like that:
...
At the beginning it was covering `Add`-button as an incremental trigger for the shown counter.
...
\noindent By now, let's just check that `Add`-button is opening a Widget with a three named tabs:
...
Here we are, by `tap` and `expect` the flow of the whole application can be checked. Such type of tests are named as 
Widget tests that's emphasized by specially named `testWidgets`-function.
...
additional library `build\_runner` (`flutter pub add build\_runner --dev`) that will generate `*.mocks.dart`-files
...
files and then proceed with `flutter test`-command):
...
\noindent So, let's automate that part as well by using the `source\_gen` package, which provides capabilities to 
generate `.dart`-files based on annotations (we've seen already - @GenerateNiceMocks). Firstly, it's needed to 
register our custom builder via `build.yaml`-file:
...
and testing framework for Behavior Driven Development approach.
...
But let's go step-by-step. So, after adding Gherkin (`flutter pub add flutter\_gherkin --dev`), we can start from a
creation of `.feature`-files to describe scenarios.
...
the original application with enabled Flutter Driver Extension (\cite{DocGh23}) that simulates user behavior. It's 
not something that we might be interested in by now, expecting their fast evaluation via simulation as it's done for 
widget tests. So, we're going to take an interpreter of `.feature`-files and use `flutter\_test`-package for all 
other stuff. An additional benefit from that is the tests visibility via Tests Terminal in IDE. It would 
work for us with all features enabled (to run/re-run a single, a group, or all tests) and we would preserve a 
single entry point to evaluate all of them.

\img{prototyping/bdd-test}{Behavioral Test evaluation}{img:p-bdd}

But let's proceed iteratively, and implement wrapper around Gherkin to parse `.feature`-files and run them 
(\cref{img:p-bdd}):
...
What can be noticed (line 20-23 for `executable\_step\_iterator.dart`-file), is that we have to declare all our 
implementations (Given/When/Then) as a list of classes. That stuff is better to be automated by scanning 
`\_steps`-folder and adding all classes into the list, and we do know already (\ref{ut-code-generator}) how to 
achieve that.
...
\noindent To hide results from low-level steps we can implement own `ProgressReporter` that will populate results only 
on failure; and by using `StringBuffer`-object override the flow of `printMessageLine` and `printMessage`-functions.
