...
And for that case we'll dive into an external package creation through IDE by pressing \key{F1} -- \q{Flutter: New 
Project} -- \q{Package}, that will generate for us only "lib" and "test"-folders together with \q{.yaml}-configuration 
file. Inside \q{lib}-folder the file is located, named identically to our package, it's an entry point for the package
