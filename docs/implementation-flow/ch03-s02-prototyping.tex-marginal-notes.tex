...
\subsection{[TBD] Taking Marginal Notes}
...
\subsubsection{Defining Application Icons}

To set an icon with background for \q{Android}:
...
mipmap-mdpi, ...) to support less than Android SDK 26 versions. Additionally, it can be changed background of loading 
screen:
...
For \q{Linux} the icons are declared from \q{my\_application.cc}-file before the line \q{gtk\_widget\_show(GTK\_WIDGET(window))}:
