...
\noindent The implementation's deference from widget tests is in a usage of the next code line, that executes tests on 
a physical device or platform:
\begin{lstlisting}
...
that affects our frames per second by providing 37 frames instead of 60 (\cref{tb:frame-build}, 
\cref{img:perf-slow-frame}):\\
...
\noindent That problem is relevant to a shader (a piece of code that runs on a graphics processing unit [GPU]) rendering
when the graphics backend (that Flutter uses) sees a new sequence of draw commands for the first time, it sometimes 
generates and compiles a custom GPU shader for that sequence of commands. This allows that sequence and potentially similar sequences to render as fast as possible.

Flutter provides command line tools for app developers to collect shaders that might be needed for end-users in the SkSL (Skia Shader Language) format. The SkSL shaders can then be packaged into the app, and get warmed up (pre-compiled) when an end-user first opens the app, thereby reducing the compilation jank in later animations.
