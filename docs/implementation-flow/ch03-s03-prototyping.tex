...
represent it, let's take a peek at "Rally" in Flutter Gallery 
...
To use the most of the available screen real estate in both height and width, it's essential to leverage the 
...
// .. closing brackets
\end{lstlisting}
\noindent That solution can be improved by the usage of \q{RotatedBox}.
...
\noindent As it might be noticed, we've injected our data into components. This helps to focus on a design 
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: Text('Navigation Menu Example')),
        drawer: Drawer(
          child: ListView(
...
Going further, the defined setter in our fake data storage (Listing \ref{code:app-data}) would help to implement 
\q{Add}-form(s) \issue{16}{bed81f5} (\cref{img:proto-form}). Flutter provides a set of widgets to display all basic 
inputs as \q{TextFormField} (for input), \q{DropdownButton} / \q{DropdownMenuItem} and \q{SearchAnchor} (for selector), 
\q{Switch} (toggler), and others. Additionally, it can be used Package Manager (\q{pub}) to apply additional components, 
as \q{Icon}-picker (\q{flutter\_iconpicker}-package), \q{Color}-selector (\q{flutter\_colorpicker}-package), and 
others. However, after combining dozens of different widgets from various libraries, we might encounter a stylistic 
inconsistency between these widgets (term, "UX Hell") and end up having to make compromises or even remove some of them
by wrapping the original components (\issue{149}{}, \issue{200}{}, \issue{204}{}, \issue{207}{}, \issue{329}{}).
...
It's important to note that the changes in the field's value are managed and monitored through \q{TextEditingController}. 
...
via \q{Color.getRandomMaterialColor()}-call):
...
\noindent As for the text, it can be truncated with an ellipsis (...) if it overflows the defined container constraints:
