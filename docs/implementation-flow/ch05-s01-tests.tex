...
that the second method was called) \q{tap}-action on buttons that leads to \q{Navigator.pop | push}-command 
...
performance degradation after a couple minutes, as illustrated in \cref{gr:taken-cycle} "Initial State". That 
fix has to go upfront of any other changes; ignoring to do so could result in a significant loss of users who will 
get their first impression in a negative aspect. DevTools has given to us an exhaustive information about the source 
of the problem, and just a couple of hours resulted in a gain of 1 ms at the start but without any significant forecast 
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Endurance Testing: Taken Cycles of Items Creation vs. Spent Time} \label{gr:taken-cycle}
\end{figure}
