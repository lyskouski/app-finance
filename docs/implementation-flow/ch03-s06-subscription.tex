...
\noindent For iOS/macOS, we should update the \q{Runner/Info.plist}-file by adding the following lines (or, by using 
Xcode's Capabilities panel to turn on Apple Pay, and select the merchant IDs we want to use; this sets the 
\q{com.apple.developer.in-app-payments} key, the value for this key is an array of merchant IDs):
...
\noindent And for Android, the change in the \q{android/app/build.gradle}-file would be:
