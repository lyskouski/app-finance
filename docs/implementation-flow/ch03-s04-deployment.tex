...
something like `minified:iP`; and our restoring from a transactions is failing. We might assume it to fix in the 
following direction:
...
build (even within one platform) will use different notation for the same classes. So, the right way would be to adjust 
...
Regarding deployment, while we are following a Serverless approach, which means there is no any dedicated server to 
