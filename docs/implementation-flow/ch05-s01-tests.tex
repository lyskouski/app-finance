...
by pressing \q{[F1]} and typing \q{devtools} there.
...
\noindent The problem is linked to a \q{tap}-action on buttons that leads to \q{Navigator.pop | push}-command 
execution, and it can be "easily" resolved by adding a duration freeze:
...
\noindent Additionally raised a problem with \q{FloatingActionButton}-buttons:
...
and destination widgets. So, we have to make them unique by adding \q{heroTag}-property per each.

Lastly, the outcomes gleaned from our exhaustive endurance testing have illuminated the presence of memory 
leaks during the intensive application usage, as depicted in Figure \ref{img:memory-profiler}. Simply saying, each 
new interaction within the application has a discernible impact on its responsiveness, culminating in a noticeable 
performance degradation after a couple minutes, as illustrated in Graph \ref{gr:taken-cycle}. That fix has to go upfront
of any other changes; ignoring to do so could result in a significant loss of users who will get their first impression 
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    grid style=dashed,
    xtick=data,
    xticklabels={10th, 20th, 40th, 100th, 200th},
  ]
  \addplot[
    color=blue,
    mark=square,
    ...
    \label{plot1}
    \addlegendentry{Spent Time}
  
  \end{axis}
\end{tikzpicture}
\caption{Endurance Testing: Taken Cycle vs. Spent Time} \label{gr:taken-cycle}
\end{figure}
