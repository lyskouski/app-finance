...
Dart is a relatively strongly typed language, which means it encourages declaring explicit types rather than relying 
on \q{var}. This practice enhances code readability and maintainability by making the intended type of each variable 
clear.

\begin{lstlisting}
String txt = 'Demo string'; // Explicit type declaration
\end{lstlisting}

For collection types that hold other objects, we can also specify the type of the stored elements.
...
\noindent The \q{DateFormat} function is used to convert and parse dates into a specific format, such as 'yyyy-MM-dd' 
...
Functions can also be composed, allowing for the creation of more complex behavior by combining simpler functions. 
...
\subsubsection{Metaprogramming Overview}
...
\noindent 1. Reflection: Dart provides a \q{dart:mirrors} library for reflection, allowing you to inspect and manipulate 
objects at runtime. However, this library is not supported in Flutter for web and mobile.

\noindent 2. Code Generation: Dart supports code generation through build_runner and source_gen packages. You can create 
custom annotations and use code generators to produce boilerplate code.

\noindent 3. Macros: Dart has introduced a macro system that allows you to write code that generates other code at 
compile time.

\noindent 4. "Magic methods": Dart allows to override certain methods in classes to change their behavior, 
such as \q{noSuchMethod()}.
...
invoke \q{noSuchMethod()} but only if it has been implemented in the class.
