...
\cite{Sanket19}, and an architectural mistake might cost \q{N x 30} on post-release phase, where N -- is an amount of
...
generate \q{.dart}-files based on annotations (we've seen already - @GenerateNiceMocks). Firstly, it's needed to 
...
widget tests. So, we're going to take an interpreter of \q{.feature}-files and use \q{flutter\_test}-package for all 
