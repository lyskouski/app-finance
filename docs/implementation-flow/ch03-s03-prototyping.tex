...
their analysis right after a first working solution creation (minimum viable product).
...
while the \q{BaseLineWidget} serves to represent individual items as lines \issue{2}{}.
...
enables the usage of dynamic construction for all routes, unifying the control over the route management.
...
selector), \q{Switch} (toggler), and others. Additionally, it can be used Package Manager (\q{pub}) to apply additional
