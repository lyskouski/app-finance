...
\markboth{Defining Quality Gates}{Testing Quality}
...
are named as Widget tests that's emphasized by specially named \q{testWidgets}-function.
...
\noindent And, finally, the test should be adjusted to support implemented wrapper:
...
The absence of fonts discernible from the captured image earlier (\cref{img:bdd-visualization}), since \q{Foo}-font 
family is used by default in tests. To address this, we might use \q{FontLoader}-class to preload all needed fonts
